...
import { intersection } from '../../src/operation/intersection.mjs';
...
describe('intersection', () => {
    ...
    it('(prec) should return empty geometry for disjoint line strings', () => {
        ...
        assert.equal(toWKT(intersection(a, b, 0)), 'LINESTRING EMPTY');
        assert.equal(toWKT(intersection(b, a, 0)), 'LINESTRING EMPTY');
    });

    it('(prec) should return intersection of two overlapping line strings', () => {
        const a = fromWKT('LINESTRING(0 0, 10 0)');
        const b = fromWKT('LINESTRING(0 1, 10 1)');
        assert.equal(toWKT(intersection(a, b, 10)), 'LINESTRING (0 0, 10 0)');
        assert.equal(toWKT(intersection(b, a, 10)), 'LINESTRING (0 0, 10 0)');
    });

    it('(prec) should return point intersection of two line strings', () => {
        const a = fromWKT('LINESTRING(0 0, 10 0)');
        const b = fromWKT('LINESTRING(0 1, 10 0)');
        assert.equal(toWKT(intersection(a, b, 0)), 'POINT (10 0)');
        assert.equal(toWKT(intersection(b, a, 0)), 'POINT (10 0)');
    });
    ...
    it('(prec) should return collection of intersections of two line strings', () => {
        ...
        assert.equal(toWKT(intersection(a, b, 2)), 'GEOMETRYCOLLECTION (LINESTRING (2 0, 4 0), POINT (10 0), POINT (0 0))');
        assert.equal(toWKT(intersection(b, a, 2)), 'GEOMETRYCOLLECTION (LINESTRING (4 0, 2 0), POINT (0 0), POINT (10 0))');
    });

    it('(prec) should return intersection of polygon with contained point', () => {
        const a = fromWKT('POLYGON((1 1,1 5,5 5,5 1,1 1))');
        const b = fromWKT('POINT(2 2)');
        assert.equal(toWKT(intersection(a, b, 0)), 'POINT (2 2)');
        assert.equal(toWKT(intersection(b, a, 0)), 'POINT (2 2)');
    });

    it('(prec) should return intersection of two empty polygons', () => {
        const a = fromWKT('POLYGON EMPTY');
        const b = fromWKT('POLYGON EMPTY');
        assert.equal(toWKT(intersection(a, b, 0)), 'POLYGON EMPTY');
        assert.equal(toWKT(intersection(b, a, 0)), 'POLYGON EMPTY');
    });
    ...
    it('(prec) should return intersection of multi polygon and polygon', () => {
        ...
        assert.equal(toWKT(intersection(a, b, 0)), 'POLYGON ((0 2, 2 2, 2 1, 0 1, 0 2))');
        assert.equal(toWKT(intersection(b, a, 0)), 'POLYGON ((2 2, 2 1, 0 1, 0 2, 2 2))');
    });
    ...
    it('(prec) should return intersection of two overlapping polygons', () => {
        ...
        assert.equal(toWKT(intersection(a, b, 1e-2)), 'POLYGON ((2.12 2.12, 2.12 1.12, 1.12 1.12, 1.12 2.12, 2.12 2.12))');
        assert.equal(toWKT(intersection(b, a, 1e-2)), 'POLYGON ((1.12 1.12, 1.12 2.12, 2.12 2.12, 2.12 1.12, 1.12 1.12))');
    });

    it('(prec) should return collection of intersections of multi polygon and polygon', () => {
        const a = fromWKT('MULTIPOLYGON(((0 0,5 10,10 0,0 0),(1 1,1 2,2 2,2 1,1 1),(100 100,100 102,102 102,102 100,100 100)))');
        const b = fromWKT('POLYGON((0 1,0 2,10 2,10 1,0 1))');
        assert.equal(toWKT(intersection(a, b, 0)), 'GEOMETRYCOLLECTION (POLYGON ((1 2, 1 1, 0.5 1, 1 2)), POLYGON ((9.5 1, 2 1, 2 2, 9 2, 9.5 1)), LINESTRING (1 2, 2 2), LINESTRING (2 1, 1 1))');
        assert.equal(toWKT(intersection(b, a, 0)), 'GEOMETRYCOLLECTION (POLYGON ((9 2, 9.5 1, 2 1, 2 2, 9 2)), POLYGON ((0.5 1, 1 2, 1 1, 0.5 1)), LINESTRING (1 2, 2 2), LINESTRING (2 1, 1 1))');
    });

    it('(prec) should throw on unsupported geometry type', () => {
        const a = fromWKT('CIRCULARSTRING (0 0, 1 1, 2 0)');
        const b = fromWKT('LINESTRING (1 0, 2 1)');
        assert.throws(() => intersection(a, b, 0), {
            name: 'GeosError::UnsupportedOperationException',
            message: 'Curved geometry types are not supported.',
        });
    });
