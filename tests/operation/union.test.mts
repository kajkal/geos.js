...
import { union } from '../../src/operation/union.mjs';
...
describe('union', () => {
    ...
    it('(prec) should return union of two points', () => {
        ...
        assert.equal(toWKT(union(a, b, 1)), 'MULTIPOINT ((2 8), (4 10))');
        assert.equal(toWKT(union(b, a, 1)), 'MULTIPOINT ((4 10), (2 8))');
    });
    ...
    it('(prec) should return union of two not completely adjacent polygons', () => {
        ...
        assert.equal(toWKT(union(a, b, 1)), 'POLYGON ((10 5, 5 5, 5 10, 10 10, 10 15, 15 15, 15 7, 10 7, 10 5))');
        assert.equal(toWKT(union(b, a, 1)), 'POLYGON ((15 15, 15 7, 10 7, 10 5, 5 5, 5 10, 10 10, 10 15, 15 15))');
        assert.equal(toWKT(union(a, b, 0.01)), 'POLYGON ((10 5, 5 5, 5 10, 10.01 10, 10 15, 15 15, 15 7, 10.01 7, 10.01 8.85, 10 5))');
        assert.equal(toWKT(union(b, a, 0.01)), 'POLYGON ((15 15, 15 7, 10.01 7, 10.01 8.85, 10 5, 5 5, 5 10, 10.01 10, 10 15, 15 15))');
    });
    ...
    it('(prec) should return union of two partially overlapping polygons', () => {
        ...
        assert.equal(toWKT(union(a, b, 1e-2)), 'POLYGON ((92.87 100, 94.73 100, 100 100, 100 88.87, 80.21 100, 85.56 100, 92.87 100))');
        assert.equal(toWKT(union(b, a, 1e-2)), 'POLYGON ((85.56 100, 92.87 100, 94.73 100, 100 100, 100 88.87, 80.21 100, 85.56 100))');
    });

    it('(prec) should throw on unsupported geometry type', () => {
        const a = fromWKT('CIRCULARSTRING (0 0, 1 1, 2 0)');
        const b = fromWKT('LINESTRING (1 0, 2 1)');
        assert.throws(() => union(a, b, 0), {
            name: 'GeosError::UnsupportedOperationException',
            message: 'Curved geometry types are not supported.',
        });
    });
