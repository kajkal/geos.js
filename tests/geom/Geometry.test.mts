...
describe('Geometry', () => {
    ...
    it('should set geometry type on construction', () => {
        ...
        const a = new GeometryRef(1001 as Ptr<GEOSGeometry>, 'Polygon');
        assert.equal(a.type, 'Polygon');
        assert.equal(typeId.mock.callCount(), 0);

        // call GEOSGeomTypeId when type is unknown
        const b = new GeometryRef(1002 as Ptr<GEOSGeometry>);
    ...
    it('should set geometry extra attributes', () => {
        ...
        let g = new GeometryRef(1001 as Ptr<GEOSGeometry>, 'Point');
        assert.equal(g.id, undefined);
        assert.equal(g.props, undefined);

        g = new GeometryRef(1001 as Ptr<GEOSGeometry>, 'Point', {});
        assert.equal(g.id, undefined);
        assert.equal(g.props, undefined);

        g = new GeometryRef(1001 as Ptr<GEOSGeometry>, 'Point', { id: 1 });
        assert.equal(g.id, 1);
        assert.equal(g.props, undefined);

        g = new GeometryRef(1001 as Ptr<GEOSGeometry>, 'Point', { properties: { some: 'prop' } });
        assert.equal(g.id, undefined);
        assert.deepEqual(g.props, { some: 'prop' });

        g = new GeometryRef(1001 as Ptr<GEOSGeometry>, 'Point', { id: 1, properties: { some: 'prop' } });
        assert.equal(g.id, 1);
        assert.deepEqual(g.props, { some: 'prop' });
    });
